"""RAG, agent, and embedding tests.

Tests text chunking, hybrid search, LLM-based reranking, the
end-to-end chat agent workflow with citations, and LLM fallback
construction with exhausted-key filtering.
"""
...
def mock_gemini(monkeypatch):
    ...
    monkeypatch.setattr(
        GoogleGenerativeAIEmbeddings, "embed_query", lambda self, text: [0.1] * 768
    )
    ...
    monkeypatch.setattr(
        Chroma, "similarity_search_with_relevance_scores", mock_similarity_search
    )
    yield
...
async def test_hybrid_search(db_session, test_users):
    ...
    assert (
        "notice" in results[0]["content"].lower()
        or "leave" in results[0]["content"].lower()
    )
...
async def test_get_llm_filters_exhausted_keys(monkeypatch):
    ...
    monkeypatch.setattr(
        ChatGoogleGenerativeAI, "ainvoke", mock_ainvoke_with_selective_failure
    )
...
def test_clean_content():
    ...
    assert (
        clean_content([{"type": "text", "text": "foo"}, "bar", {"type": "other"}])
        == "foobar"
    )
